...
// ==============================================
// CSVHandler — Phase 2
// Import and Export UI handler for expenses CSV files
// ==============================================
...
import { exportExpensesCSV, importExpensesCSV } from "@/app/(dashboard)/expenses/actions";
...
export function CSVHandler({ filters, onSuccess }: CSVHandlerProps) {
  ...
  const parseCSV = (text: string): Record<string, string>[] => {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      // Basic comma-splitting supporting quotes (very simple regex)
  ...
  const handleImport = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Map parsed CSV headers (case-insensitive) to schema
      const mappedRows = parsedRows.map((row) => {
        ...
        return {
          ...
          Amount: findVal("amount", "value", "cost") ?? "0",
          Date: findVal("date", "time", "created") ?? "",
          Currency: findVal("currency", "curr") ?? "INR",
          Category: findVal("category", "type") ?? "",
          Account: findVal("account", "wallet", "source") ?? "",
          Merchant: findVal("merchant", "vendor", "store") ?? "",
      ...
      const result = await importExpensesCSV(mappedRows);
      ...
      toast.success(
        `Imported ${result.data?.imported ?? 0} expenses successfully.`,
        { id: toastId }
      );
      setIsOpen(false);
      setFile(null);
        ...
        <DialogContent className="sm:max-w-md">
          <form onSubmit={handleImport}>
            <DialogHeader>
              ...
              <DialogDescription>
                Upload a CSV file containing your expenses. Columns should ideally include:
                <code className="block mt-1.5 p-1.5 rounded bg-muted font-mono text-[10px] text-foreground">
                  Date, Title, Amount, Currency, Category, Account, Merchant, Tags, Notes
                </code>
              </DialogDescription>
            </DialogHeader>
